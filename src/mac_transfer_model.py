...
from mac_assetto_corsa_env import MacAssettoCorsaEnvironment, create_mac_assetto_corsa_env
...
def adapt_model_for_mac_assetto_corsa(model, assetto_env):
    """Adapt the model to work with Mac Assetto Corsa environment"""
    print("Adapting model for Mac Assetto Corsa...")
    
    # Set the new environment
    model.set_env(assetto_env)
    
    # The model architecture should be compatible since we designed
    # the Mac Assetto Corsa environment to match the gym environment:
...
def test_model_on_mac_assetto_corsa(model, env, num_episodes=5):
    """Test the adapted model on Mac Assetto Corsa environment"""
    print(f"Testing model on Mac Assetto Corsa for {num_episodes} episodes...")
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            # Print progress
            if step_count % 50 == 0:
                print(f"  Step {step_count}: Action={action}, Reward={reward:.3f}, Speed={info['speed']:.1f}, On Track={info['on_track']}, Total={total_reward:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description='Transfer DQN model from gym to Mac Assetto Corsa')
    ...
    parser.add_argument('--save-path', type=str,
                       default='models/mac_assetto_corsa_dqn_model.zip',
    ...
    parser.add_argument('--simulation-mode', action='store_true', default=True,
                       help='Use simulation mode (no real screen capture)')
    ...
    try:
        ...
        # Step 2: Create Mac Assetto Corsa environment
        print("\n=== Step 2: Creating Mac Assetto Corsa Environment ===")
        assetto_env = MacAssettoCorsaEnvironment(
        ...
        adapted_model = adapt_model_for_mac_assetto_corsa(gym_model, assetto_env)
        
        # Step 4: Test the model (optional)
        if not args.no_test:
            print("\n=== Step 4: Testing Model ===")
            test_rewards = test_model_on_mac_assetto_corsa(adapted_model, assetto_env, args.test_episodes)
        ...
        if not args.no_test:
            print("Model tested on Mac Assetto Corsa simulation - check the results above.")
